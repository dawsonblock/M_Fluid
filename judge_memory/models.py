...
class EvidenceRecord(BaseModel):
    ...
    source_title: Optional[str] = Field(
        default=None, description="Human-readable title"
    )
    content_preview: Optional[str] = Field(
        default=None, description="First 1000 chars of content for search"
    )
...
class ClaimRecord(BaseModel):
    ...
    claim_type: str = Field(
        default="fact", description="Type: fact, ruling, opinion, etc."
    )
    ...
    person_id: Optional[str] = Field(
        default=None, description="Associated person/entity ID"
    )
...
class SourcePacket(BaseModel):
    ...
    authority: float = Field(
        default=0.5, ge=0.0, le=1.0, description="Source authority score"
    )
    verifiability: float = Field(
        default=0.5, ge=0.0, le=1.0, description="How verifiable the source is"
    )
    originality: float = Field(
        default=0.5, ge=0.0, le=1.0, description="Originality vs derivative"
    )
    independence: float = Field(
        default=0.5, ge=0.0, le=1.0, description="Independence from bias"
    )
    legal_status_label: str = Field(
        default="unverified", description="Legal classification"
    )
    legal_status_weight: float = Field(
        default=0.5, ge=0.0, le=1.0, description="Legal status numerical weight"
    )
...
class JudgeMemorySearchResult(BaseModel):
    ...
    result_type: str = Field(description="Type: evidence or claim")
    ...
    status: Optional[str] = Field(
        default=None, description="Claim status if applicable"
    )
    ...
    def summary(self) -> str:
        """Backward-compatible alias for content_preview."""
        return self.content_preview

    @property
    def fluid_score(self) -> Optional[float]:
        """Optional fluid score exposed through metadata when available."""
        value = self.metadata.get("fluid_score")
        return value if value is not None else None


class GroundedMemoryPacket(BaseModel):
    """A search result enriched with full provenance and trust chain.

    Returned by ``JudgeMemoryService.search_grounded()``.  Combines the raw
    search result with source-authority metadata and optional fluid activation
    so callers have everything needed for explainable, citation-ready output.
    """

    result: JudgeMemorySearchResult = Field(description="Underlying search result")
    source_packet: Optional[SourcePacket] = Field(
        default=None, description="Full source provenance packet"
    )
    fluid_activation: Optional[float] = Field(
        default=None,
        ge=0.0,
        description="Fluid memory activation score for the node (0=cold, 1=hot)",
    )
    trust_chain: List[str] = Field(
        default_factory=list,
        description="Ordered list of source IDs that corroborate this result",
    )
    grounding_metadata: Dict[str, Any] = Field(
        default_factory=dict,
        description="Arbitrary provenance annotations added during grounding",
    )
