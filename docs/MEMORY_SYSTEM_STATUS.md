...
- ✅ Source trust profiles (hardcoded)
- ✅ Keyword search (title, URL, content preview)
- ✅ Claim search by text
- ✅ Timeline queries
- ✅ Orphaned claim prevention (validates evidence_id exists)

**Features (Partial/Documented):**
- ⚠️ Fluid memory integration — calls m_flow/memory/fluid/ when available, hardcoded profiles when not
...
**Core Features:**
- ✅ Memory storage with salience/confidence/volatility
- ✅ Decay lanes (time, trust, interest, attention)
- ✅ Retrieval with fluid scoring
- ✅ Reinforcement and contradiction tracking

**Known Issues (Being Fixed):**
- Uses MD5 hashing (should be SHA256)
- Uses Python hash() for some IDs (should be UUID4/ULID)
- Some tests weak/pass silently
