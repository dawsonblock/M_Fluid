...
def test_apply_state_delta_int_fields():
    """Test applying state delta to int fields via reinforcement_count."""
    # Note: reinforcement_count is actually protected - use metadata as dict instead
    memory = MemoryItem(content="Test")
    memory.metadata = {"custom_counter": 5}

    delta = {"metadata": {"custom_counter": 10}}
    applied = apply_state_delta(memory, delta, resistance=0.0)

    # Metadata merges, so custom_counter should be updated
    assert memory.metadata["custom_counter"] == 10
